...
class ConvBN(nn.Module):
    ...
    #     flat = tensor.reshape(-1)               # view ↔ no copy
    ...
    def flip_sign_(self, tensor: torch.Tensor, percentage: float) -> torch.Tensor:
        ...
        if percentage >= 1.0:
            tensor.mul_(-1)
            print(f"Flipped 100 % of elements ({tensor.numel()}/{tensor.numel()})")
            return tensor
        ...
        flat       = tensor.reshape(-1)              # no copy
        before     = flat.clone()                 # snapshot for verification ❶
        idx        = torch.randint(0, numel, (num_to_flip,),
                                   device=flat.device)
    
        flat[idx] *= -1                           # in-place flip
    
        # ── verification ────────────────────────────────────────────────
        changed = (flat != before)                # Boolean mask
        flipped = changed.sum().item()            # how many actually flipped
        # print(f"Requested {num_to_flip}/{numel} "
        #       f"({num_to_flip/numel:.2%}) — verified {flipped} "
        #       f"({flipped/numel:.2%}) elements changed.")
        # ────────────────────────────────────────────────────────────────
    ...
    def apply_one(self, x_p, weight):
        return F.conv1d(x_p, weight, padding=0)
