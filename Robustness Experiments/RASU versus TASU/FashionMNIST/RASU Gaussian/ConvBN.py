...
class ConvBN(nn.Module):
    def __init__(self, 
        ...
        # self.bias_trick_par = (torch.tensor(0.00005))
    ...
    def init_hdc(self, ratio, seed, flip_perc=None):
        ...
        g = (torch.randn(self.nHDC, *w_bn.shape[1:], device=w_bn.device, dtype=w_bn.dtype))
        alpha1 = torch.sign((w_bn * g).sum(dim=(2, 3, 4), keepdim=True))
        if flip_perc is not None and flip_perc > 0.0:
            self.flip_sign_(alpha1, flip_perc)
