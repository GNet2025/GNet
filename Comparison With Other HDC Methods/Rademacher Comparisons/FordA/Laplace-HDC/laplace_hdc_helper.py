...
def load_dataset(dataname, selected_labels, data_folder, num_train=None, num_test=None):
    ...
    assert dataname in ["MNIST", "FashionMNIST", "CIFAR10"], "ERROR: Dataname is not valid!"
    ...
    if dataname == "FashionMNIST":
        ...
        if dataname == "CIFAR10":
            trainmnist = datasets.CIFAR10(data_folder, train=True, download=True, transform=transforms.ToTensor(),)
            testmnist = datasets.CIFAR10(data_folder, train=False, download=True, transform=transforms.ToTensor(),)
