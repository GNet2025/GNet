...
def _empty_patch(shape, *args, **kwargs):
    # if somebody passed dtype=None, drop it
    if kwargs.get("dtype", "___") is None:
        kwargs.pop("dtype", None)
    # if somebody passed device=None, drop it
    if kwargs.get("device", "___") is None:
        kwargs.pop("device", None)
    return _orig_empty(shape, *args, **kwargs)

torch.empty = _empty_patch
...
def cnn_sgd(trainloader, num_classes, epochs, sz):
    ...
    class BasicCNN(nn.Module):
        ...
        def __init__(self, n1, n2):
            ...
            self.linear = nn.Linear(in_features=int(N), out_features=10)
...
def float_sgd(trainloader, num_classes, epochs, hyperdim):
    ...
    class Model(torch.nn.Module):
        def __init__(
            ...
            # built the linear model w/ given parameters
            self.linear = nn.Linear(
                in_features=int(hyperdim), out_features=int(num_classes), bias=False
            )
