...
def predict_(am, img, position_table, grayscale_table, dim):
    qhv = encode(img, position_table, grayscale_table, dim) # encoded hypervector for the single image
    pred = 0
...
def retrain(am, X_retrain, Y_retrain, position_table, grayscale_table, dim):
    ...
    for idx in range(len(X_retrain)):
        ...
        if pred != Y_retrain[idx]: # if prediction is wrong:
            am_[pred] = np.subtract(am_[pred], qhv) # subtract the image hypervector from the wrong prediction class hypervector
            am_[Y_retrain[idx]] = np.add(am_[Y_retrain[idx]], qhv) # add the image hypervector to the correct class hypervector
    return am_
