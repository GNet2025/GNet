...
class ConvBN(nn.Module):
    ...
    def init_hd(self, ratio, seed):
        ...
        self.g = torch.sign(torch.randn(self.nHD, *w_bn.shape[1:], device=w_bn.device, dtype=w_bn.dtype))
        alpha1 = torch.sign((w_bn * self.g).sum(dim=(2, 3, 4), keepdim=True))
    ...
    def hd(self, x):
        ...
        zhat = (torch.pi / (2 * self.nHD)) * torch.sign(out).sum(dim=1)
        return zhat
