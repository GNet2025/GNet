...
class LinearBN(nn.Module):
    def __init__(self, in_features, out_features, eps=1e-4, q=1e-4, seed = 13):
        super().__init__()
        ...
        nn.init.normal_(self.linear.weight, mean=0.0, std=0.6) #Ali changed
        self.eps = eps
        self.q = q
    ...
    def init_hd(self, ratio, seed):
        ...
        n = w_comp.size(1)
        self.nHD = int(self.custom_round(ratio * n)) if ratio<1000 else int(ratio)
        torch.manual_seed(seed)
        self.g = torch.sign(torch.randn(w_comp.size(1), self.nHD)).to(torch.half).to(w_comp.device)
    ...
    def hd(self, x):
        ...
        x = x @ self.wg * (torch.pi / (2 * self.nHD))
