...
class LinearBN(nn.Module):
    def __init__(self, in_features, out_features, eps=1e-4, q=1e-4, std_l=0.1, seed_l = 13, seed_bn=14):
        super().__init__()
        ...
        torch.manual_seed(seed_l)
        nn.init.normal_(self.linear.weight, mean=0.0, std=std_l) #Ali changed
        torch.manual_seed(seed_bn)
        nn.init.normal_(self.bn.weight, mean=0.0, std=0.1) #Ali changed
        self.eps = eps
        self.q = q
