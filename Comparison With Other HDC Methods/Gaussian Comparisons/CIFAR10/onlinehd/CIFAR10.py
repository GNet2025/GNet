...
def load():
    ...
    x = torch.from_numpy(train_ds.data).reshape(-1, 32*32*3).float() / 255.0
    y = torch.tensor(train_ds.targets).long()

    x_test = torch.from_numpy(test_ds.data).reshape(-1, 32*32*3).float() / 255.0
    y_test = torch.tensor(test_ds.targets).long()
